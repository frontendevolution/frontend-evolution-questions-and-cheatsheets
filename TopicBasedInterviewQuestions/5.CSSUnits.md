...
It depends entirely on the property. Width percentages resolve against the parent's width, height percentages need an explicit height on the parent to resolve against, and things like padding-top as a percentage are famously relative to the parent's _width_, not height. There's no single rule — you have to know the resolution behavior per property.
