...
Portals escape the React tree and the DOM tree, but not stacking contexts — if any ancestor of the portal's _call site_ wraps the app in a `transform` or `opacity` (common with Framer Motion page-transition wrappers), that can create app-wide stacking issues depending on where the portal root itself lives in the DOM. The fix is making sure the portal target is a sibling at the very end of `<body>`, outside any animated wrapper, with its own explicit stacking context.
